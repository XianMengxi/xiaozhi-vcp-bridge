...
const { appendHistory, getAgentConfig } = require('./historyManager');
const TTSHandler = require('./ttsHandler');

// Mock moduleConfig and settings for the bridge environment
const moduleConfig = {
    APP_DATA_ROOT_IN_PROJECT: path.join(__dirname, 'data'),
    getMusicState: null // Music control not implemented in bridge
};

// Ensure data directory exists
fs.ensureDirSync(moduleConfig.APP_DATA_ROOT_IN_PROJECT);
 ...
 */
async function saveConversation(messages, response, context) {
    try {
        const { agentId, topicId } = context || {};
        if (!agentId || !topicId) {
            console.warn('[VCPClient] Skipping save: agentId or topicId missing in context');
            return;
        }
        ...
        if (agentId) {
            ...
            if (config && config.name) agentName = config.name;
        }

        if (lastUserMsg) {
            newRecords.push({
                role: 'user',
                name: process.env.USER_DISPLAY_NAME || 'User',
                content: lastUserMsg.content,
                timestamp: Date.now(),
                id: `msg_${Date.now()}_user_${Math.random().toString(36).substr(2, 7)}`,
                attachments: []
            });
        }
        ...
        if (response) {
            newRecords.push({
                ...
                content: response,
                timestamp: Date.now(),
                id: `msg_${Date.now()}_assistant_${Math.random().toString(36).substr(2, 7)}`,
                agentId: agentId,
                finishReason: 'completed'
            });
        }

        await appendHistory(agentId, topicId, newRecords);
    } catch (e) {
        console.error('[VCPClient] Error saving conversation:', e);
    }
}
 ...
 * @param {object} params.ws - WebSocket connection to the client (replaces webContents)
 * @param {string} params.streamChannel - 流式数据频道名称
 * @param {function} [params.onStreamEnd] - (optional) Callback for when stream ends
...
async function sendToVCP(params) {
    const {
        ...
        messageId,
        context = null,
        ws = null, // Changed from webContents
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (modelConfig.stream === true && ws && ws.readyState === 1) { // WebSocket.OPEN is 1
                ...
                ws.send(JSON.stringify({ type: streamChannel, payload: errorPayload }));
        ...
        if (modelConfig.stream === true) {
            ...
            const reader = response.body.getReader(); // Node.js fetch might return a stream differently? 
            // In Node 18+ native fetch, body is a ReadableStream.
            // However, getReader() is for web streams. Node's fetch implementation (Undici) supports standard web streams.

            const decoder = new TextDecoder();
            const ttsHandler = new TTSHandler(ws, context);
            if (ws) ws.activeTTSHandler = ttsHandler;
            ...
            if (ws && ws.readyState === 1) {
                ws.send(JSON.stringify({
                    type: 'tts',
                    state: 'start',
                    session_id: ws.sessionId // Use session_id from ws object
                }));
            }
                ...
                try {
                    while (true) {
                        ...
                        for (const line of lines) {
                            ...
                            if (line.startsWith('data: ')) {
                                ...
                                if (jsonData === '[DONE]') {
                                    ...
                                    if (ws && ws.readyState === 1) {
                                        ws.send(JSON.stringify({ type: streamChannel, payload: donePayload }));
                                    }
                                    if (onStreamEnd) {
                                        onStreamEnd({ success: true, content: accumulatedResponse });
                                    }
                                    // Save conversation on completion
                                    saveConversation(messages, accumulatedResponse, context);
                                    await ttsHandler.flush(); // Flush remaining TTS

                                    // Send TTS Stop
                                    // ttsHandler.sendStopSignal();
                                    return;
                                }
                                ...
                                try {
                                    ...
                                    if (textToAppend) {
                                        ...
                                        ttsHandler.processChunk(textToAppend); // Feed to TTS
                                    }

                                    const dataPayload = { type: 'data', chunk: parsedChunk, messageId: messageId, context };
                                    if (ws && ws.readyState === 1) {
                                        ws.send(JSON.stringify({ type: streamChannel, payload: dataPayload }));
                                    }
                                } catch (e) {
                                    ...
                                    if (ws && ws.readyState === 1) {
                                        ws.send(JSON.stringify({ type: streamChannel, payload: errorChunkPayload }));
                                    }
                                }
                            }
                        }
                        ...
                        if (done) {
                            ...
                            if (ws && ws.readyState === 1) {
                                ws.send(JSON.stringify({ type: streamChannel, payload: endPayload }));
                            }
                            if (onStreamEnd) {
                                onStreamEnd({ success: true, content: accumulatedResponse });
                            }
                            // Save conversation on completion (if not already saved by [DONE])
                            saveConversation(messages, accumulatedResponse, context);
                            await ttsHandler.flush(); // Flush remaining TTS
                            // Wait for all processing to complete
                            await ttsHandler.processingQueue;
                    ...
                    if (ws && ws.readyState === 1) {
                        ws.send(JSON.stringify({ type: streamChannel, payload: streamErrPayload }));
                    }
                    if (onStreamEnd) {
            ...
            // Save conversation
            let content = '';
            if (vcpResponse.choices && vcpResponse.choices[0] && vcpResponse.choices[0].message) {
                content = vcpResponse.choices[0].message.content;
            }
            saveConversation(messages, content, context);

            // Send to client (Fix for non-streaming bug)
            if (ws && ws.readyState === 1) {
                // Simulate stream data packet for compatibility or send a specific one
                // Sending as a single data chunk then end
                const chunk = { choices: [{ delta: { content: content } }] };
                ws.send(JSON.stringify({ type: streamChannel, payload: { type: 'data', chunk, messageId, context } }));
                ws.send(JSON.stringify({ type: streamChannel, payload: { type: 'end', messageId, context } }));
            }
            ...
            if (content) {
                ...
                const ttsHandler = new TTSHandler(ws, context);
                if (ws) ws.activeTTSHandler = ttsHandler;
                ttsHandler.processChunk(content);
        ...
        if (error.name === 'AbortError') {
            ...
            if (modelConfig.stream === true && ws && ws.readyState === 1) {
                const abortPayload = { type: 'error', error: '请求已中止', messageId: messageId, context };
                ws.send(JSON.stringify({ type: streamChannel, payload: abortPayload }));
            }
        ...
        if (modelConfig.stream === true && ws && ws.readyState === 1) {
            ...
            ws.send(JSON.stringify({ type: streamChannel, payload: catchErrorPayload }));
